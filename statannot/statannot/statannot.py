    ...
    **stats_params
    ...
    if test == 'Levene':
        ...
        result = StatResult(
            ...
            u_stat,
            pval,
            u_stat / (len(box_data1)*len(box_data2))
        )
    ...
    elif comparisons_correction == 'hs':
        print(result.pval)
        result.pval = multipletests(result.pval, alpha=0.05, method='hs')[1]
        result.pval = result.pval if len(result.pval) > 1 else result.pval[0]
        print(result.pval)
        result.test_str = result.test_str + ' with Holm-Šidák correction'
...
def add_stat_annotation(ax, plot='boxplot',
    ...
    if plot == 'boxplot':
        ...
        # Create the same plotter object as seaborn's pointplot
        box_plotter = sns.categorical._PointPlotter(
            x, y, hue, data, order, hue_order,
            estimator=np.mean, ci='std', units=None, seed=42,
            orient=None, color=None, palette=None, saturation=.75,
            errcolor=".26", errwidth=None, capsize=None, dodge=True)
    ...
    box_structs = [{'box':box_names[i],
                    ...
                    'ymax':(np.mean(get_box_data(box_plotter, box_names[i])) +
                           np.std(get_box_data(box_plotter, box_names[i])) * 2.0) if
...
#         ax.set_ylim((ylim[0], max(1.03*y_stack_max, ylim[1])))
        ax.set_ylim((ylim[0], 1.1*y_stack_max))
    elif loc == 'outside':
